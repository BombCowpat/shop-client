...
1. 点击结算到创建订单页面  路由组件静态页面实现  api请求函数 vuex store保存数据（state，mutaions，actions，getters） 组件挂载后请求数据 将数据从store拿到组件  动态加载数据到组件 地址切换交互 样式绑定   买家留言数据绑定

2. 提交订单 路由组件注册  请求api接口函数  点击提交订单 通过绑定在Vue原型上的API对象上的方法发送请求  在当前trade 组件中收集订单信息(交易编号，交易信息)， 提交成功后返回订单编号  编程式导航跳转到支付页(先注册路由组件Pay PaySuccess) 路由中带着订单编号query参数  
3. 支付页信息请求api函数 页面挂载时发送数据请求（参数，订单编号） 直接把数据保存在组件中 动态加载数据 

交易编号 订单编号
4. 订单支付逻辑  
支付微信弹框 安装elementUI 安装单独引入插件    在main.js中引入弹框组件 注册弹框函数   然后在支付组件 点击事件调用函数加载弹框   安装二维码生成组件 node-qrcode   npm install -S qrcode     使用 QRCode.toDataURL(text) 函数生成二维码 ，完善弹框内容  弹框取消失败promise处理 
定时器 定时发送请求查看支付状态，支付成功后跳转到支付成功页面  查看支付状态  用户点击确认取消按钮交互  

购物车 结算（创建订单交易信息） 提交订单 立即支付 支付成功 


### day11 
1. 用户中心路由组件  header直达用户中心跳转   拆分Center组件中的 我的订单 团购订单组件 注册为center路由下二级路由组件 
2. 我的订单 api数据接口函数 组件内拿数据 ，-----动态加载数据 
3. 合并单元格 将要合并的后面3个格子放在template标签内 template 里加v-if 让后面 三个格子不显示
4. 使用自定义翻页组件  传递必要页面参数  绑定自定义翻页事件
5. elementUI 翻页组件  选择完整功能 
6. 路由守卫  在router实例上添加 全局前置守卫   
7. 路由独享守卫和组件守卫（登录有不能看到登录页面）在当前路由中配置   组件守卫在组件内配置 
8. 懒加载  图片懒加载  	还没有加载得到目标图片时, 先显示loading图片 	在<img>进入可视范围才加载请求目标图片   组件懒加载 将组件拆分打包成js函数包 只有在使用到该组件时才会加载该组件 
9. vee 插件表单验证 vee-validate@2
10. 创建vee配置 文件 、 引入到main 然后对需要验证的表单进行配置 调用配置验证函数得到验证结果（promise） true false
